...
int PQCLEAN_MCELIECE348864_CLEAN_genpoly_gen(gf *out, gf *f) {
    ...
    gf *mat = (gf*) MemPool_Alloc(((SYS_T + 1)*(SYS_T))*sizeof(gf));
    gf mask, inv, t;
    ...
    for (j = 0; j < SYS_T; j++) {
        ...
        if ( mat[ j*SYS_T + j ] == 0 ) { // return if not systematic
            MemPool_Free(((SYS_T + 1)*(SYS_T))*sizeof(gf));
            return -1;
        }
...
int PQCLEAN_MCELIECE348864_CLEAN_perm_check(const uint32_t *p) {
    int i;
    uint64_t *list = (uint64_t*) MemPool_Alloc((1 << GFBITS)*sizeof(uint64_t));
    ...
    MemPool_Free((1 << GFBITS)*sizeof(uint64_t));
